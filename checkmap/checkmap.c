...
/*   Updated: 2023/09/20 15:18:34 by ogorfti          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cone_data(char **split, t_obj *obj)
{
	...
	cone->axis = (t_tuple) {axis_tuple.x, axis_tuple.y, axis_tuple.z, axis_tuple.w};

	cone->center = (t_tuple) {coords_tuple.x, coords_tuple.y, coords_tuple.z, coords_tuple.w};

	obj->transform_inverse = ft_inverse(ft_multi_matrices(ft_translate(cone->center.x, cone->center.y, cone->center.z),
		ft_multi_matrices(ft_get_rotation_matrix(ft_vector(0, 1, 0), cone->axis),ft_scale(cone->d, cone->h, cone->d))));
...
void	check_count(t_data *data)
{
	...
	while (data->find[i].split)
	{
		...
		if (split[0] && !ft_strcmp(split[0], "A"))
			...
			count.l++;
		else if (split[0] && ft_strcmp(split[0], "pl") && ft_strcmp(split[0], "sp")
			&& ft_strcmp(split[0], "cy") && ft_strcmp(split[0], "cn"))
